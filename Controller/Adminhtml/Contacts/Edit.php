...
class Edit extends \Magento\Backend\App\Action
{
    ...
    protected function _initAction()
    {
        ...
        $resultPage->setActiveMenu('Iverve_CrudOperation::mycontact')
            ->addBreadcrumb(__('Manage Information'), __('Manage Information'))
            ->addBreadcrumb(__('Manage Information'), __('Manage Information'));
        return $resultPage;
    }
    ...
    public function execute()
    {
        ...
        $this->_coreRegistry->register('contacts_post', $model);

        // 5. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Info') : __('New Information'),
            $id ? __('Edit Info') : __('New Information')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Records'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getTitle() : __('New Information'));
